...
def run_verification(N=400, block_size=16):
    ...
    try:
        result = subprocess.run(
            [exe_path, str(N), str(block_size), fileA, fileB, fileC],
            capture_output=True, text=True, check=True
        )
        gpu_time = float(result.stdout.strip())
        print(f"Успешно. Время на GPU: {gpu_time:.2f} мс")
    except subprocess.CalledProcessError as e:
        print("Ошибка запуска CUDA программы!")
        print(e.stderr)
        return
    ...
    if is_correct:
        print("✅ Результаты на GPU совпадают.")
    else:
        print("❌ Результаты на GPU не совпадают.")
